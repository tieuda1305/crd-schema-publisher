...
- **Watch mode uses full re-extract.** Simpler than incremental. Upload is already incremental via check-missing. Extraction of <200 CRDs is sub-second. `SCHEMA_FILTER_*` and `--kind`/`--group`/`--version` filter the generated output for each cycle, while the informer still watches all CRDs. If an active filter matches no CRDs, runtime builds create and publish an empty catalog so stale broader output is not preserved.
